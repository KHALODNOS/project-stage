...
exports.LastViewChapter = async (req, res) => {
  const { novelId } = req.params;
  const user = req.user;
  try {
    const record = user.Lastview.find(
      (view) => view.novel.toString() === novelId,
    );
    console.log(record);
    res.status(200).json({
      lastViewedChapter: record ? record.chapterNumber : 0,
      scrollPosition: record ? record.scrollPosition : 0
    });
  } catch (error) {
...
exports.updatViewChapter = async (req, res) => {
  const { novelId } = req.params;
  ...
  try {
    const lastViewIndex = user.Lastview.findIndex(
      (view) => view.novel.toString() === novelId,
    );
    ...
    if (lastViewIndex !== -1) {
      update = {
        $set: { 
            [`Lastview.${lastViewIndex}.chapterNumber`]: chapterNumber,
            [`Lastview.${lastViewIndex}.scrollPosition`]: scrollPosition || 0
        },
      };
    } else {
      ...
      update = {
        $push: {
          Lastview: {
            $each: [{ novel: novelId, chapterNumber: chapterNumber, scrollPosition: scrollPosition || 0 }],
