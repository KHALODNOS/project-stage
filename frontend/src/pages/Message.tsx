...
const Message: React.FC = () => {
  ...
  return (
    <div className="flex h-[calc(100vh-80px)] overflow-hidden bg-background font-NotoKufi" dir="rtl" onClick={() => setEmojiPickerFor(null)}>
          ...
          >
            <div className="p-6 border-b border-border bg-muted/20">
              <div className="flex items-center justify-between mb-1">
                <h3 className="text-xl font-black text-foreground font-elmessiri">المجتمع</h3>
                <div className="p-2 glass rounded-lg text-primary">
                  <Users className="w-5 h-5" />
                </div>
              </div>
              <p className="text-xs text-muted-foreground font-bold uppercase tracking-widest">غرفة الدردشة العامة</p>
            </div>
            ...
            <div className="flex-1 overflow-y-auto px-4 py-6 space-y-6">
              <div>
                <div className="flex items-center gap-2 mb-4 px-2">
                  ...
                  <span className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em]">متصل الآن — {onlineUsers.length}</span>
                </div>
                <div className="space-y-1">
                  {onlineUsers.length === 0 ? (
                    ...
                    onlineUsers.map((u) => (
                      <div key={u.socketId} className="flex items-center gap-3 p-3 rounded-2xl hover:bg-accent transition-all group">
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-bold text-foreground truncate">{u.nickname}</p>
                          <span className={cn(
            ...
            <div className="p-4 bg-muted/20 border-t border-border flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-black text-foreground truncate">{user.nickname}</p>
      ...
      <main className="flex-1 flex flex-col relative bg-background">
        ...
        {/* Top Glass Bar */}
        <header className="h-20 flex items-center justify-between px-6 glass sticky top-0 z-20 border-b border-border">
          <div className="flex items-center gap-4">
            <button
              onClick={() => setSidebarOpen(!sidebarOpen)}
              className="p-2.5 glass rounded-xl text-muted-foreground hover:text-foreground hover:bg-accent transition-all hidden lg:block"
            >
            ...
            <div className="flex items-center gap-3 px-3 py-1.5 glass rounded-2xl bg-primary/5 border-primary/20">
              ...
              <div>
                <h2 className="text-base font-black text-foreground leading-tight">الدردشة العامة</h2>
                <p className="text-[10px] text-primary font-bold uppercase tracking-widest">{onlineUsers.length} متصل الآن</p>
              </div>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <button className="p-2.5 glass rounded-xl text-muted-foreground hover:text-foreground transition-all">
            ...
            grouped.map((group) => (
              <div key={group.date} className="space-y-8">
                <div className="flex items-center gap-4 text-center">
                  ...
                  <span className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em] px-4 py-1.5 glass rounded-full">{group.date}</span>
                  <div className="h-px bg-border flex-1" />
                </div>
                    ...
                    >
                      ...
                      {!isOwn && (
                        <img
                          ...
                          className="w-10 h-10 rounded-2xl object-cover border border-border shrink-0"
                        ...
                        <div className="relative group">
                          <div className={cn(
                            ...
                            isOwn
                              ...
                              : "bg-muted/50 border border-border backdrop-blur-md text-foreground rounded-bl-none"
                          )}>
                            <p className="text-sm leading-relaxed whitespace-pre-wrap">{m.text}</p>
                            <div className={cn(
                              "text-[9px] mt-2 opacity-50 font-bold",
                              isOwn ? "text-white/70" : "text-muted-foreground text-left"
                            )}>
                          ...
                          <div className={cn(
                            ...
                            <button
                              ...
                              className="p-2 glass rounded-xl text-muted-foreground hover:text-primary hover:bg-accent"
                            >
                          ...
                          <AnimatePresence>
                            {emojiPickerFor === m._id && (
                              <motion.div
                                ...
                                className={cn(
                                  "absolute bottom-full mb-3 p-2 glass rounded-2xl border border-border z-[100] flex gap-2 shadow-2xl",
                          ...
                          <div className={cn("flex flex-wrap gap-1.5 mt-1 animate-in zoom-in-50", isOwn ? "justify-end" : "justify-start")}>
                            {Object.entries(reactionCounts).map(([emoji, count]) => (
                              <button
                                ...
                                className="flex items-center gap-1.5 px-2 py-1 glass rounded-full border border-border text-xs hover:bg-accent transition-colors"
                              >
                                <span>{emoji}</span>
        ...
        <div className="p-4 md:p-6 glass border-t border-border bg-card/50 backdrop-blur-xl">
          <div className="max-w-5xl mx-auto flex items-end gap-3">
            <div className="flex-1 relative group">
              <div className="absolute right-4 bottom-4 p-2 text-muted-foreground group-focus-within:text-primary transition-colors">
              ...
              <textarea
                ...
                rows={1}
                maxLength={1000}
                className="w-full bg-muted/20 border border-border rounded-[28px] py-4 pr-12 pl-14 text-foreground text-sm focus:border-primary focus:ring-1 focus:ring-primary/50 outline-none transition-all resize-none max-h-40 font-medium"
                style={{ minHeight: '56px' }}
              />
              <div className="absolute left-4 bottom-4 flex gap-2 text-muted-foreground">
